...
async function seed() {
	...
	await db.transaction(async (tx) => {
		...
		await tx.insert(invoicesTable).values(seedInvoices())
	})
}

function seedInvoices(length = 10): NewInvoice[] {
	return Array.from({ length }, () => ({
		...
		items: Array.from({ length: faker.helpers.rangeToNumber({ min: 2, max: length / 2 }) }, () => ({
